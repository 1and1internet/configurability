...
func (php *PhpParserData) PhpJsonLoader(data []byte) {
	// Set some defaults
	php.JsonData.OpCache.OpCacheMemory = "128"
	php.JsonData.OpCache.OpCacheRevalidateFreq = 2
	php.JsonData.OpCache.OpCacheEnableCli = false
...
func (php *PhpParserData) ApplyCustomisations() {
	...
	if iniFile != nil {
		...
		if err == nil {
			...
			plugins.UpdateInt64Key("PHP", opcache_section, "opcache.revalidate_freq", php.JsonData.OpCache.OpCacheRevalidateFreq)
			plugins.UpdateBoolKey("PHP", opcache_section, "opcache.enable_cli", php.JsonData.OpCache.OpCacheEnableCli)
		}
